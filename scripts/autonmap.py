...
def autonmap(host_ips):
    nm = nmap.PortScanner()
    nm.scan(hosts = host_ips, arguments= '-T4 --open -Pn -n -p 22,80,443,5432,389,636')
    ...
    for host in nm.all_hosts():
        if nm[host].state() == 'up':
            ...
            for proto in nm[host].all_protocols():
                ...
                for port in lport:
                    if port == 80 and not "Apache" in nm[host][proto][version]:
                        continue
                    elif port in services and not port == 80:
